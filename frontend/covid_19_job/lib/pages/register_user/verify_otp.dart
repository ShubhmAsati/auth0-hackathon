...
class _ScaffoldWidgetState extends State<ScaffoldWidget> {
  ...
  void confirmOTP() {
    ...
    rg.VerifyRegisterOtp(requestPayload).then((value) {
      if (value['error'].toString().isEmpty) {
        snackBar(true,"Otp Verified");
        Navigator.pushNamedAndRemoveUntil(
            context, value['nextPage'], (route) => false);
      } else {
        snackBar(false,value['error']);
  ...
  snackBar(isOtpVerified,message) {
    if (isOtpVerified) {
  ...
  resetOtp() {
    ...
    RegisterController rg = new RegisterController();
    String userId = dataFromPreviousPage['userId'];
    rg.ResendOtp(userId).then((value) {
      if (value['error'].toString().isEmpty) {
